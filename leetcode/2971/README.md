# [2971. Find Polygon With the Largest Perimeter (Medium)](https://leetcode.com/problems/find-polygon-with-the-largest-perimeter)
...
**Companies**:
[Airtel](https://leetcode.com/company/airtel)
...
```cpp
// OJ: https://leetcode.com/problems/find-polygon-with-the-largest-perimeter
...
public:
    long long largestPerimeter(vector<int>& A) {
        sort(begin(A), end(A));
        long long N = A.size(), ans = -1, sum = 0;
        for (int i = 0; i < N; ++i) {
            if (i > 1 && sum > A[i]) ans = sum + A[i];
            sum += A[i];
        }
        return ans;
    }
