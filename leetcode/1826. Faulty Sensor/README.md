...
```cpp
// OJ: https://leetcode.com/problems/faulty-sensor/
// Author: github.com/lzl124631x
// Time: O(N)
// Space: O(1)
class Solution {
    bool valid(vector<int> &A, vector<int> &B) { // returns true if `A` is the faulty sensor
        int N = A.size(), i = 0;
        while (i < N && A[i] == B[i]) ++i;
...
public:
    int badSensor(vector<int>& A, vector<int>& B) {
        bool a = valid(A, B), b = valid(B, A);
        return a + b == 1 ? (a ? 1 : 2) : -1;
    }
};
```