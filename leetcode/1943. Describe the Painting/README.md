...
```cpp
// OJ: https://leetcode.com/problems/describe-the-painting/
// Author: github.com/lzl124631x
// Time: O(N + M) where M is maximum end time.
// Space: O(M)
class Solution {
public:
    vector<vector<long long>> splitPainting(vector<vector<int>>& A) {
        ...
        int last = 0;
        for (auto &a : A) {
            int s = a[0], e = a[1], c = a[2];
            sum[s] += c;
            sum[e] -= c;
            change[s] = change[e] = true;
            last = max(last, e);
        }
        ...
        for (int i = 0; i <= last; ) {
            int start = i;
            while (i <= last && sum[i] == sum[start] && (i == start || !change[i])) ++i;
            if (sum[start]) {
                ans.push_back({ start, i, sum[start] });
            }
        }
        return ans;
    }
