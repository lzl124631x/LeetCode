...
Let the maximum element be `mx`, and the sum of the rest elements be `sum`.

If `mx <= sum`, we can always finish all the milestones, the answer is `mx + sum`.

If `mx > sum`, we can only finish `sum * 2 + 1` milestones.
