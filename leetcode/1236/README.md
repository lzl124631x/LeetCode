...
**Companies**:
[Bloomberg](https://leetcode.com/company/bloomberg), [Microsoft](https://leetcode.com/company/microsoft), [Dropbox](https://leetcode.com/company/dropbox), [Facebook](https://leetcode.com/company/facebook)

**Related Topics**:  
[String](https://leetcode.com/tag/string), [Depth-First Search](https://leetcode.com/tag/depth-first-search), [Breadth-First Search](https://leetcode.com/tag/breadth-first-search), [Interactive](https://leetcode.com/tag/interactive)

**Similar Questions**:
* [Web Crawler Multithreaded (Medium)](https://leetcode.com/problems/web-crawler-multithreaded)
...
```cpp
// OJ: https://leetcode.com/problems/web-crawler
...
public:
    vector<string> crawl(string startUrl, HtmlParser htmlParser) {
        ...
        auto origin = getOrigin(startUrl);
        while (q.size()) {
            auto url = q.front();
            q.pop();
            auto urls = htmlParser.getUrls(url);
            for (auto &next : urls) {
                if (seen.count(next) == 0 && next.starts_with(origin)) {
                    seen.insert(next);
                    q.push(next);
                }
            }
        }
        return vector<string>(begin(seen), end(seen));
    }
};
```