# [3062. Winner of the Linked List Game (Easy)](https://leetcode.com/problems/winner-of-the-linked-list-game)
...
```cpp
// OJ: https://leetcode.com/problems/winner-of-the-linked-list-game
...
public:
    string gameResult(ListNode* head) {
        int cnt = 0;
        while (head) {
            cnt += head->val > head->next->val ? 1 : -1;
            head = head->next->next;
        }
        return cnt > 0 ? "Even" : (cnt < 0 ? "Odd" : "Tie");
    }
};
```
