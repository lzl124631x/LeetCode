# [2861. Maximum Number of Alloys (Medium)](https://leetcode.com/problems/maximum-number-of-alloys)
...
```cpp
// OJ: https://leetcode.com/problems/maximum-number-of-alloys
// Author: github.com/lzl124631x
// Time: O(KNlogM) where M is the maximum possible number of alloys produced
// Space: O(1)
class Solution {
public:
    int maxNumberOfAlloys(int n, int k, int budget, vector<vector<int>>& P, vector<int>& S, vector<int>& C) {
        long ans = 0;
        auto valid = [&](int m, long cnt) {
            long cost = 0;
            for (int i = 0; i < n && cost <= budget; ++i) {
                cost += max(0L, cnt * P[m][i] - S[i]) * C[i];
            }
            return cost <= budget;
        };
        for (int i = 0; i < k; ++i) {
